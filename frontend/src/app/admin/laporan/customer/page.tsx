...
import { getAdminCustomerReport, CustomerReportData } from "@/utils/api";
...
export default function AdminCustomerReportPage() {
  ...
  const [activeParams, setActiveParams] = useState<{ start_date: string; end_date: string }>({
    start_date: getFirstDayOfMonthStr(),
    end_date: getTodayStr(),
  });

  const { data, isLoading, isFetching, isError, error } = useQuery({
    queryKey: ["adminCustomerReport", activeParams.start_date, activeParams.end_date],
    queryFn: () => getAdminCustomerReport(activeParams),
    enabled: !!activeParams.start_date && !!activeParams.end_date,
    staleTime: 60000,
    placeholderData: (prev) => prev,
  });

  const reportData: CustomerReportData | undefined = data?.data;
  ...
  const setQuickRange = (range: "this_month" | "last_month" | "last_30" | "this_year") => {
    ...
    setEndDateInput(end);
    setActiveParams({ start_date: start, end_date: end });
  };
    ...
    <AdminClientGuard>
      ...
      >
        ...
        <div className="print:hidden">
          <AdminSidebar activeTab="laporan-customer" isDarkMode={isDarkMode} />
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="sticky top-0 z-30 shrink-0 print:hidden">
            <AdminHeader
              ...
              subtitle="Statistik & Rekapitulasi Aktivitas Belanja Customer Berdasarkan Database Aktual"
          ...
          <div className="hidden print:block pb-4 mb-6 border-b-2 border-black">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm font-bold text-gray-800 tracking-wide mt-1">LAPORAN AKTIVITAS CUSTOMER</p>
              </div>
          ...
          >
            ...
            <div style={{ marginBottom: "24px" }} className="print:hidden flex flex-wrap justify-between items-center gap-4">
              <div>
                ...
                >
                  REKAPITULASI CUSTOMER PERIODE
                </h2>
                <p style={{ fontSize: "12px" }} className={`font-mono mt-1 ${isDarkMode ? "text-[#666666]" : "text-[#9CA3AF]"}`}>
                  Filter dan cetak rekap data customer bertransaksi dari database
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={handlePrint}
                disabled={!reportData || reportData.summary.total_customers === 0}
                style={{
                  padding: "10px 20px",
                  borderRadius: "7px",
                  fontSize: "12px",
                  ...
                  alignItems: "center",
                  gap: "8px",
                  opacity: (!reportData || reportData.summary.total_customers === 0) ? 0.4 : 1,
                }}
            ...
            {reportData && (
              ...
              >
                <div className="flex items-center gap-2 font-mono">
                  ...
                  <span>
                    PERIODE LAPORAN: <strong className="text-[#B6A47E]">{reportData.period.start_fmt}</strong> S/D <strong className="text-[#B6A47E]">{reportData.period.end_fmt}</strong>
                  </span>
                </div>
                <div style={{ fontSize: "11px" }} className="font-mono text-[#8A8A8A] print:hidden">
                  * HANYA MENAMPILKAN CUSTOMER DENGAN PESANAN VALID (NON-CANCELLED).
                </div>
              </div>
            )}
            ...
            {!isError && reportData && (
              <>
                ...
                <div style={{ marginBottom: "28px" }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <AdminStatCard
                    label="CUSTOMER BERTRANSAKSI"
                    value={`${reportData.summary.total_customers} CUSTOMER`}
                    subtext="Memiliki transaksi valid di periode ini"
                    icon={UserCheck}
                    accentColor="#B6A47E"
                    isDarkMode={isDarkMode}
                  />
                  <AdminStatCard
                    ...
                    subtext="Akumulasi seluruh customer"
                    icon={ShoppingBag}
                    accentColor={isDarkMode ? "#F5F5F5" : "#0A0A0A"}
                    isDarkMode={isDarkMode}
                  />
                  <AdminStatCard
                    label="TOTAL BELANJA CUSTOMER"
                    value={formatIDR(reportData.summary.total_spent)}
                    subtext="Akumulasi pengeluaran customer"
                    icon={DollarSign}
                    accentColor="#B6A47E"
                    isDarkMode={isDarkMode}
                  />
                ...
                {reportData.summary.total_customers === 0 ? (
                  ...
                  >
                    ...
                    <h4 style={{ fontSize: "14px", fontWeight: 700 }} className="uppercase tracking-wider">BELUM ADA DATA CUSTOMER</h4>
                    <p style={{ fontSize: "12px" }} className="text-[#8A8A8A] max-w-md font-mono">
                      Belum ada data customer pada periode yang dipilih ({reportData.period.start_fmt} - {reportData.period.end_fmt}). Silakan pilih rentang tanggal lain.
                    </p>
                  </div>
                ) : (
                  /* Standard Order Table Layout Matching Admin Customers Control Page */
                  <div
                  ...
                  >
                    ...
                    <div style={{ padding: "16px 20px" }} className="border-b flex items-center justify-between border-inherit print:hidden">
                      <div>
                        <h4 style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "0.08em" }} className="uppercase font-mono">
                          RINCIAN AKTIVITAS CUSTOMER
                        </h4>
                        <p style={{ fontSize: "11px" }} className="text-[#8A8A8A] font-mono mt-0.5">
                          Menampilkan {reportData.customers.length} customer aktif bertransaksi
                        </p>
                      </div>
                    </div>
                    ...
                    >
                      <table className="w-full min-w-[750px] text-left text-xs uppercase tracking-wider">
                        ...
                        >
                          <tr>
                            <th style={{ width: "5%", padding: "14px 16px" }} className="font-bold whitespace-nowrap">NO.</th>
                            <th style={{ width: "20%", padding: "14px 16px" }} className="font-bold whitespace-nowrap">NAMA CUSTOMER</th>
                            <th style={{ width: "27%", padding: "14px 16px" }} className="font-bold whitespace-nowrap">EMAIL</th>
                            <th style={{ width: "14%", padding: "14px 16px" }} className="font-bold text-center whitespace-nowrap">PESANAN VALID</th>
                            <th style={{ width: "16%", padding: "14px 16px" }} className="font-bold text-center whitespace-nowrap">TRANSAKSI TERAKHIR</th>
                            <th style={{ width: "18%", padding: "14px 16px" }} className="font-bold text-right whitespace-nowrap">TOTAL BELANJA</th>
                          </tr>
                        </thead>
                        <tbody
                          className={`divide-y ${
                          ...
                          {reportData.customers.map((cust, idx) => (
                            ...
                            >
                              <td
                                ...
                                className="font-mono font-bold text-[#8A8A8A] whitespace-nowrap"
                              >
                                {idx + 1}.
                              </td>
                              <td
                              ...
                              <td
                                style={{ padding: "14px 16px" }}
                                className="font-mono text-[#8A8A8A] lowercase whitespace-nowrap"
                              >
                                {cust.customer_email}
                              </td>
                              <td style={{ padding: "14px 16px" }} className="text-center font-mono font-bold whitespace-nowrap">
                                {cust.valid_orders_count} PESANAN
                              </td>
                              <td style={{ padding: "14px 16px" }} className="text-center font-mono text-[#8A8A8A] whitespace-nowrap">
                                {cust.last_order_fmt}
                              </td>
                              <td
                                style={{ padding: "14px 16px" }}
